...
1. La pregunta central es sobre frecuencias, no sobre posiciones
   ...
   * La naturaleza del problema: Al problema no le importa dónde está el número ni en qué orden aparece. Solo importa: "¿Cuántas veces aparece el número $X$ en la lista A frente a la lista B?". Cuando el orden no influye y solo importan las repeticiones de cada valor, la estructura ideal es una tabla de conteo/frecuencias (map o unordered_map).
     
2. El análisis de complejidad temporal ($O(N \cdot M)$ vs. $O(N + M)$)
   * Tu código: Por cada uno de los $N$ elementos de lista1, ejecutas std::find() sobre lista2, que en el peor caso revisa sus $M$ elementos. Esto da un costo de tiempo de $O(N \cdot M)$. Si $N, M \approx 10^5$, harías alrededor de $10^{10}$ operaciones (el límite estándar por segundo en jueces online suele ser $\approx 10^8$, lo que causará Time Limit Exceeded).
