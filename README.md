...
2. El análisis de complejidad temporal (O(N . M) vs. O(N + M))
   * Tu código: Por cada uno de los $N$ elementos de `lista1`, ejecutas `std::find()` sobre `lista2`, que en el peor caso revisa sus M elementos. Esto da un costo de tiempo de O(N . M). Si N, M aprox $10^5$, harías alrededor de $10^{10}$ operaciones (el límite estándar por segundo en jueces online suele ser aprox $10^8$, lo que causará Time Limit Exceeded).
   * Con `unordered_map`: Insertar y consultar toma tiempo promedio O(1). Procesar ambas listas requiere recorrerlas una sola vez: O(N + M) (aprox 2 . $10^5$ operaciones), resolviéndose en milisegundos.
